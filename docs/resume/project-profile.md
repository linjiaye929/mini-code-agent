...
> 、M5b host-pinned local stdio MCP 已发布，M6a host-profiled read-only analysis
> Subagent 已完成本地实现与安全门。Shell 字符串、项目可执行 Hook、OS 沙箱、remote
> HTTP/OAuth MCP、自动 Repair Resume、write-capable Subagent/Worktree 和真实凭证联调
> 尚未实现。`v0.14.0-alpha.0` GitHub prerelease 已发布；`0.15.0a0` 当前最终本地
> Python 3.13 为 1062 passed、10 个 Windows symlink 条件跳过、91.09% 分支覆盖率，
> Python 3.12 的最终 release rerun、artifact、CI、tag 和 Release 尚待执行。
>
